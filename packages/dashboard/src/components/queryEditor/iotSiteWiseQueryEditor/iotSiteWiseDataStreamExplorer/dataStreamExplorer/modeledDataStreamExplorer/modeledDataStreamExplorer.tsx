...
export interface ModeledDataStreamExplorerProps extends WithIoTSiteWiseClient {
  selectedDataStreams: SelectedDataStream[];
  onSelect: (dataStreams: SelectedDataStream[]) => void;
}

export function ModeledDataStreamExplorer({ selectedDataStreams, client }: ModeledDataStreamExplorerProps) {
  ...
  const { assetProperties, isFetching: isFetchingAssetProperties } = useAssetProperties({
    assetIds: selectedDataStreams.map(({ assetId }) => assetId),
    client,
  });
